...
interface WelcomeScreenProps {
  ...
  onSelectSpeed: (speed: 'smart' | 'fast' | 'mega' | 'image' | 'vision') => void;
  user?: any;
...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = React.memo(({
  ...
  const displayName = user?.displayName?.split(' ')[0] || userName?.split(' ')[0] || 'Davi';

  return (
    <div className="flex flex-col items-center justify-center w-full max-w-3xl px-4 py-8 mx-auto animate-fadeIn duration-500">
      
      {/* Header Greeting */}
      <div className="flex flex-col items-center text-center mb-8">
        <ZenoLogo size={56} variant={logoVariant} theme={theme} className="mb-6 shadow-xs" />
        <h1 className={\`text-3xl sm:text-4xl font-extrabold tracking-tight mb-3 \${
          isDark ? 'text-white' : 'text-neutral-900'
        }\`}>
          Olá, {displayName}!
        </h1>
        <p className="text-sm sm:text-base text-neutral-400 max-w-md font-medium">
      ...
      {showSuggestions ? (
        ...
        <div className="w-full mt-4 flex flex-col items-center max-w-2xl">
          ...
          {recentSessions && recentSessions.length > 0 ? (
              ...
              {recentSessions.slice(0, 4).map(session => (
                <button
                  ...
                  className={\`flex items-start gap-3 p-4 rounded-2xl border text-left transition-all \${
                    isDark
                      ? 'bg-[#1a1a1e] hover:bg-[#222228] border-[#2E2E2E]'
                      : 'bg-white hover:bg-neutral-50 border-neutral-200'
                  }\`}
                >
                  <MessageSquare className="w-5 h-5 text-neutral-400 mt-0.5 flex-shrink-0" />
            ...
            <p className="text-sm text-neutral-500 py-4">Nenhuma conversa ainda.</p>
          )}
        </div>
      )}
    </div>
