...
import { 
  ...
  FileText, FileCode, Image as ImageIcon, Loader2, MessageSquare
} from 'lucide-react';
...
export const MessageItem = React.memo<MessageItemProps>(({
  ...
  const handleToggleMoreMenu = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleToggleFeedbackMenu = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleShare = useCallback(async () => {
    ...
    await onCopy(msg.id, msg.text);
    setIsSharing(false);
    setSharedSuccess(true);
    setTimeout(() => setSharedSuccess(false), 2000);
  }, [msg.id, msg.text, onCopy]);

  const handleFeedbackClick = useCallback((type: 'up' | 'down') => {
    onSetFeedback(msg.id, type);
    setShowFeedbackTags(true);
    setSelectedTags([]);
    setFeedbackSubmitted(false);
  }, [msg.id, onSetFeedback]);
  ...
  }, [msg.id, itemFeedback, selectedTags, onSendAdaptiveFeedback]);
  if (msg.role === 'user') {
    return (
      <motion.div 
        ...
        className="group flex w-full justify-end px-3 sm:px-4 py-2" 
  ...
  return (
    <motion.div 
      ...
      className="group flex flex-col w-full px-3 sm:px-4 py-6 border-b border-neutral-100/5 dark:border-white/5 last:border-b-0" 
      style={{ contain: 'content' }}
    >
              ...
              {(!msg.text && isLoadingLast && msg.role === 'model') ? (
                ...
                <div className="py-2">
                   {msg.isToolCalling ? (
                     ...
                     <div className="flex items-center gap-2.5 text-zeno text-sm font-medium animate-pulse">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 opacity-90 group-hover:opacity-100 transition-opacity">
              ...
              <div className="relative">
                <motion.button
                  ...
                  className={`p-1.5 rounded-lg transition-colors flex items-center justify-center text-xs active:scale-95 ${
                ...
                {showFeedbackMenu && feedbackCoords && createPortal(
                  ...
                  >
                    ...
                    <div className="h-px bg-neutral-500/10 my-1" />
                    <button
                      onClick={() => {
                        setShowFeedbackMenu(false);
                      ...
                      className="w-full text-left px-3 py-2 text-xs hover:bg-neutral-500/10 flex items-center gap-2 transition-transform active:scale-95"
                    >
                      <MessageSquare className="w-3.5 h-3.5 text-zeno" strokeWidth={2} />
                ...
                <div>
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-bold bg-zeno text-[#F5F5F5] hover:bg-zeno/10 transition-colors flex items-center gap-1.5 shadow-md shadow-zeno/10 active:scale-95"
                    >
