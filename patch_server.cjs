...
// Insert after other API routes, maybe near the bottom before app.use(vite.middlewares)
const supportCode = `
// Support Chat API
...
app.post("/api/support/chat", async (req, res) => {
  try {
    ...
    const kbPath = path.join(__dirname, 'src/lib/faqKnowledge.ts');
    let kb = "";
...
server = server.replace(/(\/\/ Vite middleware for development)/, supportCode + "\n$1");
